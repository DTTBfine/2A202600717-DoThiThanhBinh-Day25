...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    """Average time from circuit OPEN to CLOSED across all breakers, in ms."""
    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                recovery_ms = (float(entry["ts"]) - open_ts) * 1000.0
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    logger.emit("scenario.start", name=scenario.name, description=scenario.description)
    ...
    for _ in range(config.load_test.requests):
        ...
        if result.route == "static_fallback":
            ...
            # covers "primary", "cache_hit:X.XX"
            metrics.successful_requests += 1

        if result.latency_ms > 0:
            metrics.latencies_ms.append(result.latency_ms)

    # Count circuit open events from transition logs
    for breaker in gateway.breakers.values():
    ...
    logger.emit(
        ...
        circuit_opens=metrics.circuit_open_count,
        recovery_ms=metrics.recovery_time_ms,
